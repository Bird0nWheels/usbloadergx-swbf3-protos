...
that launches the cancelled **Star Wars Battlefront 3** (Free
...
The SWBF3 dev builds all ship with the same placeholder disc ID
`RABAZZ` and title "Sample Game Name". USB Loader GX identifies games
by ID, so without renaming, every build you have collapses to a single
menu entry. The custom IDs `RSBE3A` / `RSBE3B` / `RSBE3C` are arbitrary
